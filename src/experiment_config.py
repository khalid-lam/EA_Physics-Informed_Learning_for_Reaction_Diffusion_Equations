...
class ExperimentConfig:
    ...
    # Equation & model
    equation_name: str  # "poisson", "fisher", or "allen_cahn"
    model_name: str     # "mlp", "fourier", "fourier_nl", or "ResNet"
...
def create_model_factory(device: torch.device, dtype: torch.dtype) -> Dict[str, Callable]:
    ...
    return {
        ...
        # Lower-frequency Fourier basis for nonlinear PDE residual training.
        'fourier_nl': lambda: LinearFourierModel(
            input_dim=2,
            max_freq=6
        ).to(device),
...
def create_allen_cahn_configs() -> List[ExperimentConfig]:
    """
    Curated experiment configs for Allen-Cahn equation.
    
    Returns:
        List of ExperimentConfig objects for Allen-Cahn experiments
    """
    return [
        ExperimentConfig(
            name="MLP residual + noisy data",
            equation_name="allen_cahn",
            model_name="mlp",
            use_energy=False,
            w_pde=1.0,
            w_bc=1.0,
